				...
				struct cpuidle_driver *drv)
{
	...
	for (i = 0; i < drv->state_count; i++)
		d->log2_tres[i] =
			nap_fx_log2(max_t(u64, ((u64)drv->states[i].target_residency * NSEC_PER_USEC), 1));
...
{
	u64 deepest_lat = ((u64)drv->states[drv->state_count - 1].exit_latency * NSEC_PER_USEC);
...
static void nap_update(struct nap_cpu_data *d, struct cpuidle_driver *drv)
{
	...
	for (k = 1; k < drv->state_count; k++)
		if (d->learn_actual_ns >= ((u64)drv->states[k].target_residency * NSEC_PER_USEC))
			label_bin = k;
		      ...
		      struct cpuidle_device *dev, s64 latency_req)
{
	...
	for (k = m; k >= 1; k--) {
		if (dev->states_usage[k].disable)
			continue;
		if (((u64)drv->states[k].exit_latency * NSEC_PER_USEC) > (u64)latency_req)
			continue;
		idx = k;
		break;
	}
				    ...
				    s64 latency_req)
{
	...
	for (i = 1; i < drv->state_count; i++) {
		if (dev->states_usage[i].disable)
			continue;
		if (((u64)drv->states[i].exit_latency * NSEC_PER_USEC) > (u64)latency_req)
			continue;
		return i;
	}
	return 0;
		      ...
		      bool *stop_tick)
{
	...
	if (min_state == 0 ||
		...
		/*
		 * 4.14: struct cpuidle_device has no poll_limit_ns field, and
		 * poll_state is not built on arm64, so the upstream POLL
		 * timeout hint is inert here and simply omitted.
		 */
	...
	if (((drv->states[idx].flags & CPUIDLE_FLAG_POLLING) ||
		...
		*stop_tick = false;
		...
		while (idx > 0 &&
		       ((u64)drv->states[idx].target_residency * NSEC_PER_USEC) > delta_tick) {
			idx--;
...
static void nap_reflect(struct cpuidle_device *dev, int index)
{
	...
	/* 4.14: no dev->last_state_idx; the core passes @index directly. */
	...
	d->stats.total_residency_ns += measured_ns;
	if (index > 0 && measured_ns < ((u64)drv->states[index].target_residency * NSEC_PER_USEC))
