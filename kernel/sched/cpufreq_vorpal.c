 ...
 *
 * Backported from the GKI 5.10 tree (Steambot12 / Templar-Kernel-GKI-5.10)
 * with the handful of sched-core helpers that were refactored into
 * kernel/sched on the 5.10 side. On 4.14 everything this governor needs from
 * the scheduler lives below and is kept in one place.
 *
 ...
 * applied by rfx_get_util(), so this only raises the resting OPP. ---- */
...
/* Thermal poller state (deferrable, sleeps -> never in the util hook). */
static struct thermal_zone_device *rfx_tz;
static char rfx_tz_name[THERMAL_NAME_LENGTH] = "thermal_zone0";
...
/*
 * Forward declaration: the per-policy init needs the governor name before the
 * full definition lower in the file (mirrors the GKI 5.10 layout).
 */
static struct cpufreq_governor vorpal_gov;

/* ===================================================================== */
/* Sched-core helpers (kept here: the GKI 5.10 exports these from         */
/* kernel/sched/cpufreq_schedutil.c, but 4.14 has no such hooks).         */
/* ===================================================================== */

/*
 * The raw CFS signal alone is blind to three things that dominate this
 * platform:
 *
 ...
 *   DL util - reserved DEADLINE bandwidth that must be granted, and
 *             schedtune - the cgroup margin the framework already applies.
 *
 * This mirrors schedutil_cpu_util(FREQUENCY_UTIL) on the GKI side: take the
 * boosted CFS+RT util, fold in IRQ/steal corrections, then add the 25% DVFS
 * headroom that 4.14 keeps inside map_util_freq() -- a call this governor
 * deliberately does not make (it maps util->freq itself).
 */
static void rfx_get_util(struct rfx_cpu *rfx_c, unsigned long boost)
{
	unsigned long util;

	sched_avg_update(cpu_rq(rfx_c->cpu));
	util = boosted_cpu_util(rfx_c->cpu, NULL);

	/* Boot margin is per-CPU capacity, not an absolute constant. */
	util = util + (util >> 2);

	rfx_c->util = min(util, arch_scale_cpu_capacity(NULL, rfx_c->cpu));
	rfx_c->bwmin = 0;
}

/*
 * DEADLINE bandwidth that appeared since the last eval must break the rate
 * gate or a bandwidth-bound render cluster stalls on the daily period. There
 * is no cpu_bw_dl() on 4.14, so bwmin is 0 and the check below always trips
 * once a runnable DL task shows up -- conservative, and safe.
 */
static inline void rfx_ignore_dl_rate_limit(struct rfx_cpu *rfx_c)
{
	struct dl_rq *dl_rq = &cpu_rq(rfx_c->cpu)->dl;
...
/*
 * Slow-path worker class: SCHED_DEADLINE with fake bandwidth, mirroring
 * sugov_kthread_create(). A DL task needs the clock this worker is about to
 * raise, so it must not be preemptible by one, and an RT worker shares rt_rq
 * bandwidth a runaway vendor RT thread can throttle.
 */
static int rfx_setattr_sugov(struct task_struct *t)
{
...
/*
 * 4.14 has no cpufreq_this_cpu_can_update(); the caller's CPU is serialized
 * by rq->lock, and the policy this CPU belongs to cannot have its update gone
 * remote because our hook lives on the same policy->cpus the commit needs.
 */
static inline bool rfx_this_cpu_can_update(struct cpufreq_policy *policy)
{
...
static int rfx_ntiers(void)
{
	...
	for_each_possible_cpu(cpu) {
		unsigned long c = arch_scale_cpu_capacity(NULL, cpu);
 ...
 * applies to. 4.14 has no arch_scale_thermal_pressure(), so the only live
 * throttle channel is policy->max -- the vendor thermal HAL. Measured against
 * the high-water policy->max, not the live value: a statically low baseline
 * (per-core OPP split) must read as no throttle.
 */
static unsigned int rfx_thermal_headroom_pct(struct rfx_policy *p,
					     unsigned int *baseline)
{
	struct cpufreq_policy *pol = p->policy;
	unsigned int clamp_pct = 100;
				    ...
				    unsigned long max_cap, u64 time, bool gaming)
{
	 ...
	 * it. fmax is the unthrottled baseline. */
	unsigned int fmax;
	unsigned int fceil;
	...
	fceil_pct = rfx_thermal_headroom_pct(p, &fmax);
	if (unlikely(!fmax))
		return pol->cur;
...
static void rfx_iowait_boost(struct rfx_cpu *rfx_c, u64 time, unsigned int flags)
{
	 ...
	 * of a completion. No gaming tier: it enters util before the DVFS
	 * margin, so it outranked every gaming floor while blind to the idle
...
static bool rfx_should_update_freq(struct rfx_policy *p, u64 time)
{
	...
	if (unlikely(!p || !p->policy))
		return false;
	if (!rfx_this_cpu_can_update(p->policy))
		return false;
...
static void rfx_work(struct kthread_work *work)
{
	 ...
	 * work_lock -- opposite order, deadlock. */
...
/*
 * Highest CPU number in the possible mask. 4.14 has no cpumask_last().
 * CPU numbering ascends with capacity on every DynamIQ part, so the last
 * possible CPU is always the top (Prime) cluster.
 */
static unsigned int rfx_possible_last_cpu(void)
{
	unsigned int cpu, last = nr_cpu_ids;
...
{
	return cpumask_test_cpu(rfx_possible_last_cpu(), policy->related_cpus);
}
...
/*
 * DVFS worker for the slow path (any driver without fast_switch -- on the
 * SM6150 boards this governor ships on, mediatek-cpufreq, so every commit goes
 * through here). SCHED_DEADLINE, not SCHED_FIFO: a DL task needs the clock
 * this worker is about to raise, so it must not be preemptible by one, and an
 * RT worker shares rt_rq bandwidth a runaway vendor RT thread can throttle.
 */
...
static int rfx_start(struct cpufreq_policy *policy)
{
	...
	/* Re-derive roles on a topology that is now final: an early init()
	 * saw unnormalized capacities (all 1024), so trust the last reading. */
	p->is_prime = rfx_cap_is_prime(arch_scale_cpu_capacity(NULL,
						cpumask_first(policy->cpus)));
	p->is_little = rfx_cap_is_little(arch_scale_cpu_capacity(NULL,
						cpumask_first(policy->cpus)));
...
MODULE_LICENSE("GPL");
MODULE_ALIAS("cpufreq-governor:vorpal");
