...
static void do_freq_limit(struct sugov_policy *sg_policy, unsigned int *freq)
{
	...
	if (*freq > sg_policy->tunables->efficient_freq[sg_policy->tunables->current_step] && !sg_policy->first_hp_request_time) {
		...
		*freq = sg_policy->tunables->efficient_freq[sg_policy->tunables->current_step];
		...
		/* It's already under current efficient frequency -> Go to lower step */
		...
		/* Hold frequency restriction within current horizon step */
		*freq = sg_policy->tunables->efficient_freq[sg_policy->tunables->current_step];
		...
		/* Horizon shift to a higher allowed step */
				...
				unsigned int next_freq)
{
	...
	if (policy->fast_switch_enabled) {
		...
		if (next_freq == CPUFREQ_ENTRY_INVALID || !next_freq)
			return;
				  ...
				  unsigned long util, unsigned long max)
{
	...
	/* Restored original Schedhorizon square-root calculation logic */
...
static void sugov_get_util(unsigned long *util, unsigned long *max, int cpu)
{
	...
	*max = cfs_max;
	...
	/* Bound execution profile metric with active task uclamp layout rules */
...
static unsigned int *resolve_data(const char *buf, int *num_ret, size_t count)
{
	...
	cp = buf;
	i = 0;
	while (i < num && (size_t)(cp - buf) < count) {
					...
					const char *buf, size_t count)
{
	...
	if (new_efficient_freq) {
		...
		if (old != default_efficient_freq_lp && old != default_efficient_freq_perf)
			kfree(old);
	}
					...
					const char *buf, size_t count)
{
	...
	if (new_up_delay) {
		...
		tunables->current_step = 0;
		if (old != default_up_delay_lp && old != default_up_delay_perf)
			kfree(old);
	}
