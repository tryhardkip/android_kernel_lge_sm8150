 ...
 *
 * This contains the implementation of Clang's Profile Guided Optimization
 * (PGO) instrumentation runtime, backported for this tree.
 */
...
/*
 * This lock guards both profile counters and value profiling data structures.
 * The counter update path uses this lock too, so it must not be held for long.
 */
...
void __llvm_profile_instrument_range(u64 target_value, void *data,
				     ...
				     s64 precise_last, s64 large_value)
{
	if (large_value != S64_MAX && (s64)target_value >= large_value)
