 ...
 *
 * This contains the file system used to expose the Clang PGO profiling data
 * to user space via debugfs (/sys/kernel/debug/pgo/), backported for this
 * tree.
 */
...
static u32 __prf_get_value_size(struct llvm_prf_data *p, u32 *value_kinds)
{
	...
	for (kind = 0; kind < ARRAY_SIZE(p->num_value_sites); kind++) {
		...
		for (n = 0; n < sites; n++) {
			u32 count = 0;
			struct llvm_prf_value_node *site =
				nodes[s + n];
...
static void prf_serialize_value(struct llvm_prf_data *p, void **buffer)
{
	...
	for (kind = 0; kind < ARRAY_SIZE(p->num_value_sites); kind++) {
		...
		*buffer += prf_get_value_record_site_count_size(sites);
		...
		/*
		 * If we don't have nodes, we can skip serializing the actual
		 * data.
		 */
		if (!nodes)
			continue;

		/* Serialize the value profile data. */
		for (n = 0; n < sites; n++) {
			u32 count = 0;
			   ...
			   size_t len, loff_t *pos)
{
	...
	for (data = __llvm_prf_data_start; data < __llvm_prf_data_end; data++) {
		...
		for (i = 0; i < data->num_counters; i++)
			vnodes[i] = NULL;
	}

	memset(__llvm_prf_vnds_start, 0, prf_vnds_size());
