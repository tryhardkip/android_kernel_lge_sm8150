 ...
 *
 * This contains the internal definitions used by Clang's Profile Guided
 * Optimization (PGO) instrumentation runtime, backported for this tree.
 *
 * IMPORTANT: These structures must match the layout emitted by the Clang
 * version used to build the kernel. See the compiler source file
 * llvm/include/llvm/ProfileData/InstrProfData.inc. If you build the
 * instrumented kernel with a Clang whose raw profile format differs from
 * LLVM_INSTR_PROF_RAW_VERSION below, the generated profraw will be rejected
 * by llvm-profdata. Pin a compatible toolchain for instrumented builds.
 */
 ...
 * See llvm/include/llvm/ProfileData/InstrProfData.inc.
 */
 ...
 *   counters.
 * @counters_size: the size in bytes of the counters section.
 * @padding_bytes_after_counters: the number of padding bytes after the
 *   counters.
 * @names_size: the size in bytes of the names section.
 * @counters_delta: the beginning of the counters section.
 * @names_delta: the beginning of the names section.
 * @value_kind_last: the last value profile data kind.
 */
 ...
 * @counter_ptr: a pointer to the counters section.
 * @function_ptr: a pointer to the function.
 * @values: the profiled values associated with this function.
...
#define __DEFINE_PRF_SIZE(s) \
	...
	{								\
		return __llvm_prf_ ## s ## _end - __llvm_prf_ ## s ## _start; \
	}								\
